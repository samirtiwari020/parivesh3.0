...
import { Globe, CheckCircle2, Users, XCircle, FileText, ArrowUpRight } from 'lucide-react';
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] relative bg-stone-50/50 flex flex-col font-sans -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8 py-8 lg:py-12 overflow-hidden items-center">
      ...
      <div className="max-w-7xl mx-auto w-full space-y-8 md:space-y-10 relative z-10">
        ...
        >
          <div className="px-8 py-6 border-b border-emerald-100/50 bg-white/40 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h3 className="text-xl font-black text-emerald-950 font-serif flex items-center gap-3">
                <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                All Proposals Registry
              </h3>
              <p className="text-sm font-medium text-emerald-700/70 mt-1">National view of all active and historic proposals.</p>
            </div>
          </div>
          ...
          <div className="overflow-x-auto w-full">
            <table className="w-full text-left border-collapse min-w-[800px]">
              ...
              <tbody className="divide-y divide-emerald-100/30">
                <AnimatePresence>
                  {isLoading ? (
                    <tr><td colSpan={6} className="px-8 py-16 text-center text-emerald-400 font-bold animate-pulse">Loading nationwide registry...</td></tr>
                  ) : loadError ? (
                    ...
                    >
                      <td className="px-8 py-5 font-bold text-emerald-950/80 text-sm tabular-data">{app.id}</td>
                      <td className="px-8 py-5 font-semibold text-emerald-950 truncate max-w-[250px]">{app.projectName}</td>
                      <td className="px-8 py-5 font-medium text-emerald-700/80">{app.state}</td>
                      <td className="px-8 py-5">
                        <span className="inline-flex items-center justify-center px-3 py-1 rounded-full bg-emerald-100/50 text-emerald-700 text-xs font-bold border border-emerald-200">
                          {app.clearanceType}
                        </span>
                      </td>
                      ...
                      <td className="px-8 py-5 text-right">
                        <Link 
                          ...
                          className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-emerald-50 text-emerald-600 hover:bg-emerald-600 hover:text-white hover:scale-110 hover:shadow-lg hover:shadow-emerald-500/30 transition-all font-bold group-hover:bg-emerald-100"
                        >
                    ...
                    <tr>
                      <td colSpan={6} className="px-8 py-20 text-center">
                        <div className="flex flex-col items-center justify-center text-emerald-300">
                          ...
                          <p className="font-bold text-lg text-emerald-800/60">No applications found in the national registry.</p>
                        </div>
                      </td>
                    </tr>
                  )}
